...
bool is_fibonacci(int *ptr, int len, int stride) {
    ...
    // 遍历验证每一组斐波那契三项（i最大为len-3，确保i+2 < len）
    for (int i = 0; i <= len - 3; ++i) {
        // 计算当前i、i+1、i+2位置的元素（按步长stride偏移指针）
        int a = *(ptr + i * stride);       // 第i项
        int b = *(ptr + (i + 1) * stride); // 第i+1项
        int c = *(ptr + (i + 2) * stride); // 第i+2项
        // 检查是否满足斐波那契规则，不满足则立即返回false
        if (c != a + b) {
            return false;
        }
    }
